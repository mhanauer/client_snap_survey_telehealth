...
tele_snap_dat = read.csv("TelehealthSnapMDclie_DATA_2020-06-10_1301.csv", header = TRUE, na.strings = c(""))
tele_snap_dat_complete  = subset(tele_snap_dat, my_first_instrument_complete == 2)
dim(tele_snap_dat_complete)
library(naniar)
## use pairwise
miss_var_summary(tele_snap_dat_complete)
dim(tele_snap_dat_complete)
### After removing the incomplete only found one staff who took the survey on accident
#tele_snap_dat_complete = subset(tele_snap_dat_complete, record_id != 37)
```
video_audio overall
...
video_audio_overall_dat = tele_snap_dat_complete


describe.factor(tele_snap_dat_complete$video_audio)


video_audio_overall_dat = na.omit(data.frame(video_audio_overall = tele_snap_dat_complete$video_audio))
...
title_video_audio_overalll = paste0("In the SnapMD session you just completed with your Centerstone provider, \n did you use audio only or audio and video?", " ", "n=", n_video_audio_overall)
...
1, New | 2, Returning
```{r}
return_client_overall_dat = tele_snap_dat_complete



return_client_overall_dat = na.omit(data.frame(return_client_overall = tele_snap_dat_complete$return_client))
...
```{r}
instructions_overall_dat = tele_snap_dat_complete

describe.factor(tele_snap_dat_complete$instructions)


instructions_overall_dat = na.omit(data.frame(instructions_overall = tele_snap_dat_complete$instructions))
...
```{r}
quick_overall_dat = tele_snap_dat_complete

describe.factor(tele_snap_dat_complete$quick)


quick_overall_dat = na.omit(data.frame(quick_overall = tele_snap_dat_complete$quick))
...
sat_overall_dat = tele_snap_dat_complete
...
sub_overall_dat = tele_snap_dat_complete
...
```{r}
recommend_overall_dat = na.omit(data.frame(recommend_overall = tele_snap_dat_complete$recommend))
n_recommend_overall = dim(recommend_overall_dat)[1]

recommend_overall_dat$recommend_overall = dplyr::recode(recommend_overall_dat$recommend_overall, "1" = "Definitely", "2" = "Very Likely", "3" = "Somewhat Likely", "4" = "Unlikely", "5" = "Would not recommend")
...
```{r}
expectation_overall_dat = na.omit(data.frame(expectation_overall = tele_snap_dat_complete$expectation))
...
```{r}
prefer_service_overall_dat = na.omit(data.frame(televideo = tele_snap_dat_complete$prefer_service___1, telephone = tele_snap_dat_complete$prefer_service___2, in_person = tele_snap_dat_complete$prefer_service___3))
...
prefer_service_overall_dat$response_option = dplyr::recode(prefer_service_overall_dat$response_option, "in_person" = "in person")
...
Code later
```{r}
benefits_complete_client_snap = na.omit(tele_snap_dat_complete$benefits)
length(benefits_complete_client_snap)
write.csv(benefits_complete_client_snap, "benefits_complete_client_snap.csv", row.names = FALSE)
```
barriers
code later
```{r}

barriers_complete_client_snap = na.omit(tele_snap_dat_complete$barriers)
length(barriers_complete_client_snap)
write.csv(barriers_complete_client_snap, "barriers_complete_client_snap.csv", row.names = FALSE)
...
state_overall_dat = na.omit(data.frame(state_overall = tele_snap_dat_complete$state))
n_state_overall = dim(state_overall_dat)[1]

state_overall_dat$state_overall = dplyr::recode(state_overall_dat$state_overall, "1" = "Indiana", "2" = "Florida", "3" = "Tennessee", "4" = "Illinois", "5" = "Another state")
...
65+
```{r}
age_overall = na.omit(tele_snap_dat_complete$age)
...
```{r}
gender_overall_dat = na.omit(tele_snap_dat_complete$gender)
describe.factor(gender_overall_dat)
n_gender_overall = length(gender_overall_dat)


gender_overall_dat = dplyr::recode(gender_overall_dat, "1" = "Male", "2" = "Female", "3" = "Another gender identity", "4" = "Prefer not to respond")
...
sat_clinician_dat = tele_snap_dat_complete
...
prefer_service_client_dat = na.omit(data.frame(televideo = tele_snap_dat_complete$prefer_service___1, telephone = tele_snap_dat_complete$prefer_service___2, in_person = tele_snap_dat_complete$prefer_service___3))
